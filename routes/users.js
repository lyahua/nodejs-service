...
router.post('/regist', function(req, res) {
  // 第1步：接受入参
  var { username, password, password2 ,role} = req.body  // 解构赋值
  // 第2步：验证入参
...
router.post('/login', function(req, res) {
  ...
  userModel.find({username, password,role}).then(arr=>{
    if(arr.length===1) {
      // 根据用户信息生成token
      res.json({err:0,msg:'success',data: {token: jwt.createToken({username,password,role}),role:role}})
    } else {
